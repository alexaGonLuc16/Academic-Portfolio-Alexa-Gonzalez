...
async def main():
    """Main page that displays the access log."""
    ...
    # Generate access table HTML
    access_table = "<table><tr><th>Time</th><th>User</th><th>Status</th></tr>"
    for entry in access_log:
        access_table += (
            f"<tr><td>{entry['time']}</td>"
            f"<td>{entry['user']}</td>"
            f"<td>{entry['status']}</td></tr>"
        )
    access_table += "</table>"
...
async def receive_code(request: Request):
    """Receive access code data from the client."""
    ...
    return JSONResponse(
        content={"message": "No code received"},
        status_code=400
    )
